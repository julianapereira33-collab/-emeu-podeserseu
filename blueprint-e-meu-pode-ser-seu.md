...
**Notificação (substituindo n8n + Z-API):** notificação in-app (tabela `notificacoes`, badge "você tem uma mensagem" no painel) como canal principal — sempre funciona, custo zero, e o prazo de 24h já expira sozinho no Postgres mesmo sem a vendedora ver o aviso. **Atualização 10-11/08/2026:** a Juliana montou uma VPS própria (Hostinger, `179.198.118.181`) com n8n self-hosted e Evolution API self-hosted (substituindo n8n cloud/Z-API pagos), então WhatsApp real passou a ser viável — instância dedicada `emeu-podeserseu`, conectada ao número do negócio. Implementado como canal auxiliar ao lado do in-app, disparado **direto do Postgres via `pg_net`** (migrations `fase7b_pg_net_extension` e `fase7c_whatsapp_via_pg_net`) a partir do mesmo trigger que já cria a notificação in-app — não pelo Next.js, o que cobre também o caminho automático da expiração de 24h (`expirar_reservas_vencidas`), que não passa por nenhuma rota da aplicação. Credenciais no Supabase Vault. Falta adicionar e-mail/SMS como canais de alcance adicionais.
