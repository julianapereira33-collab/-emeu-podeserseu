...
## 5. Arquitetura
Next.js (Vercel) com áreas: cadastro, vitrine pública, painel da vendedora/loja, painel da embaixadora (rede + comissões), painel admin. Supabase como banco (Postgres + RLS) e Storage real para as fotos (troca o base64 do protótipo por upload de verdade — mais rápido e mais barato). n8n orquestra: tratamento de imagem (Claude API) no cadastro, disparo de confirmação de disponibilidade via Z-API quando uma reserva é criada, lembrete automático se a vendedora não responder em 24h (e liberação automática da peça de volta à vitrine), cálculo e registro de comissões após cada pagamento confirmado, e cálculo do piso de proposta por tempo em vitrine (job diário).
...
4. n8n dispara WhatsApp automático pra vendedora: "confirma disponibilidade?" — prazo de 24h.
...
Next.js + Vercel, Supabase (dados + auth + storage + RLS), n8n Cloud (orquestração e timers de 24h), Z-API (WhatsApp, header Client-Token), Claude API (tratamento de imagem e sugestão de preço). Pagamento da taxa: Pix via gateway com suporte a valor dinâmico (validar Mercado Pago ou similar, já que o valor muda por peça e pode mudar por proposta aceita).
...
- **Z-API:** exige header `Client-Token`. **n8n:** path `/webhook/` em produção, timezone `America/Sao_Paulo` explícito no timer de 24h. **Supabase:** service role key nas automações, RLS desenhada desde a modelagem porque há muitos "donos" de dado diferentes.
...
STACK: Next.js + Vercel (site com áreas: cadastro, vitrine pública, painel da vendedora/loja, painel da embaixadora, painel admin). Supabase (Postgres + Auth + Storage real para fotos + RLS). n8n Cloud (orquestração e timers). Z-API (WhatsApp, usando header Client-Token). Claude API (tratamento de imagem e sugestão de preço).
...
2. Fluxo de reserva: compradora reserva ou propõe valor → sistema dispara WhatsApp automático para a vendedora perguntando se ainda está disponível (e se aceita a proposta, se houver) → prazo de 24h (timezone America/Sao_Paulo) → sem resposta, a peça é automaticamente liberada de volta à vitrine e a reserva marcada como recusada, SEM cobrança de taxa. Só após confirmação da vendedora a cobrança da taxa é liberada à compradora.
...
- Z-API exige o header Client-Token além da URL da instância.
- Use a service role key do Supabase nas automações do n8n, nunca a anon key.
- Use o path /webhook/ em produção no n8n, nunca /webhook-test/.
